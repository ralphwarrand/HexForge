...
namespace Hex
{
    ...
    public:
        ...
        template<typename T>
        static std::shared_ptr<T> LoadWith(const std::string& key, std::function<std::shared_ptr<T>()> loader) {
            auto& cache = GetCache<T>();
            {
                std::lock_guard lock(cache.mutex);
                auto it = cache.map.find(key);
        ...
        static std::shared_ptr<Mesh> LoadMesh(const std::string& filepath, unsigned meshIndex);

        static std::shared_ptr<Material> LoadMaterial(const std::string& key, const std::string& vs,
        ...
        static std::shared_ptr<Texture> LoadTexture(const std::string& filepath, bool srgb = true);
